...
use wrf_constants, only: u1,u2,bet,worog, domain_zonal, domain_meridional, &
                      & dlogtheta,f0,g,horog,scale_length,rossby_number
use kinds

implicit none

type(wrf_field), intent(inout) :: flds    !< Model fields
...
if (ipert/=0) then
  write(record,*)"wrf_invent_state_f90: Perturbing invented state by ",ipert,"%."
