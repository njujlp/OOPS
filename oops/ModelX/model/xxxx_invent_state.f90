...
use xxxx_constants, only: u1,u2,bet,worog, domain_zonal, domain_meridional, &
                      & dlogtheta,f0,g,horog,scale_length,rossby_number
use kinds

implicit none

type(xxxx_field), intent(inout) :: flds    !< Model fields
...
if (ipert/=0) then
  write(record,*)"xxxx_invent_state_f90: Perturbing invented state by ",ipert,"%."
