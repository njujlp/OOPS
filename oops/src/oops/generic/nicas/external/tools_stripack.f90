...
!
  ...
  u = x - int ( x )
  jrand = real ( n, kind = kind_real ) * u + 1.0_kind_real
...
!
  if ( s12 == 0.0_kind_real .or. s23 == 0.0_kind_real .or. s31 == 0.0_kind_real ) then
    areas = 0.0_kind_real
    return
  end if
...
!  Parameters:
!
!    Input, real ( kind = kind_real ) V1(3), V2(3), V3(3), the coordinates of the 
!    three triangle vertices (unit vectors) in counter clockwise order.
!
!    Output, real ( kind = kind_real ) C(3), the coordinates of the circumcenter unless
...
!
  ...
  real ( kind = kind_real ) c(3)
  real ( kind = kind_real ) cnorm
  real ( kind = kind_real ) cu(3)
  real ( kind = kind_real ) e1(3)
  real ( kind = kind_real ) e2(3)
  integer ( kind = 4 ) ier
  real ( kind = kind_real ) v1(3)
  real ( kind = kind_real ) v2(3)
  real ( kind = kind_real ) v3(3)
...
!    IER = 1.
!
!    Output, real ( kind = kind_real ) XC(2*N-4), YC(2*N-4), ZC(2*N-4), the coordinates
...
!    if 0 < NB.
!
!    Output, real ( kind = kind_real ) RC(2*N-4), the circumradii (the arc lengths or
...
!
  ...
  real ( kind = kind_real ) xc(2*n-4)
  real ( kind = kind_real ) y(n)
  real ( kind = kind_real ) yc(2*n-4)
  real ( kind = kind_real ) z(n)
  real ( kind = kind_real ) zc(2*n-4)
...
!    deletion.
!
!    Input/output, real ( kind = kind_real ) X(N), Y(N), Z(N), the coordinates of 
...
!
  ...
  real ( kind = kind_real ) x1
  real ( kind = kind_real ) x2
  real ( kind = kind_real ) xl
  real ( kind = kind_real ) xr
  real ( kind = kind_real ) y(*)
  real ( kind = kind_real ) y1
  real ( kind = kind_real ) y2
  real ( kind = kind_real ) yl
  real ( kind = kind_real ) yr
  real ( kind = kind_real ) z(*)
  real ( kind = kind_real ) z1
  real ( kind = kind_real ) z2
  real ( kind = kind_real ) zl
  real ( kind = kind_real ) zr
!
...
!
  ...
  real ( kind = kind_real ) dp12
  real ( kind = kind_real ) dp1l
  real ( kind = kind_real ) dp1r
  real ( kind = kind_real ) dp2l
  real ( kind = kind_real ) dp2r
  integer ( kind = 4 ) i
  integer ( kind = 4 ) ier
  integer ( kind = 4 ) ierr
  integer ( kind = 4 ) in1
...
!
      if ( left ( x1, y1, z1, x2, y2, z2, x(nl), y(nl), z(nl) ) ) then
        ...
        if ( ( 0.0_kind_real <= dp2l - dp12 * dp1l .or. &
               0.0_kind_real <= dp2r - dp12 * dp1r )  .and. &
             ( 0.0_kind_real <= dp1l - dp12 * dp2l .or.  &
               0.0_kind_real <= dp1r - dp12 * dp2r ) ) then
          go to 6
        end if
!
...
!
!    Output, real ( kind = kind_real ) DF, value of an increasing function (negative
...
!
  ...
  real ( kind = kind_real ) df
  real ( kind = kind_real ) dnb
  real ( kind = kind_real ) dnp
  integer ( kind = 4 ) i
  integer ( kind = 4 ) ier
  integer ( kind = 4 ) lend(*)
  integer ( kind = 4 ) list(*)
...
!
  ...
  if ( cn(1) == 0.0_kind_real .and. cn(2) == 0.0_kind_real  .and. cn(3) == 0.0_kind_real ) then
    go to 1
  end if
...
!  Parameters:
!
!    Input, real ( kind = kind_real ) X(*), Y(*), Z(*), the nodal coordinates.
!
...
!
!    Input/output, real ( kind = kind_real ) X(N), Y(N), Z(N), the components of
!    the vectors.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = kind_real ) PX, PY, PZ, the coordinates of P.
!
!    Output, real ( kind = kind_real ) PLAT, the latitude of P in the range -PI/2 
!    to PI/2, or 0 if PNRM = 0.
!
!    Output, real ( kind = kind_real ) PLON, the longitude of P in the range -PI to PI,
!    or 0 if P lies on the Z-axis.  
!
!    Output, real ( kind = kind_real ) PNRM, the magnitude (Euclidean norm) of P.
!
  implicit none

  real ( kind = kind_real ) plat
  real ( kind = kind_real ) plon
  real ( kind = kind_real ) pnrm
  real ( kind = kind_real ) px
  real ( kind = kind_real ) py
  real ( kind = kind_real ) pz

  pnrm = sqrt ( px * px + py * py + pz * pz )

  if ( px /= 0.0_kind_real .or. py /= 0.0_kind_real ) then
    plon = atan2 ( py, px )
  else
    plon = 0.0_kind_real
  end if

  if ( pnrm /= 0.0_kind_real ) then
    plat = asin ( pz / pnrm )
  else
    plat = 0.0_kind_real
  end if
