...
if (ioff/=self%nf*self%nl)  call abor1_ftn ("fv3gfs_fields:create error number of fields")
...
if (fld1%nf /= fld2%nf .or. fld1%nl /= fld2%nl) then
  call abor1_ftn("fv3gfs_fields:field_prod error number of fields")
endif
!if (fld1%lbc .or. fld2%lbc) then
!  call abor1_ftn("fv3gfs_fields:field_prod should never dot_product full state")
!endif
...
if (self%nx==rhs%nx .and. self%ny==rhs%ny) then
  ...
  call abor1_ftn("fv3gfs_fields:add_incr: not coded for low res increment yet")
endif
...
if (x1%nx==x2%nx .and. x1%ny==x2%ny) then
  if (lhs%nx==x1%nx .and. lhs%ny==x1%ny) then
    ...
    call abor1_ftn("fv3gfs_fields:diff_incr: not coded for low res increment yet")
  endif
else
  call abor1_ftn("fv3gfs_fields:diff_incr: states not at same resolution")
endif
...
if (fld%nx==rhs%nx .and. fld%ny==rhs%ny) then
  ...
  call abor1_ftn("fv3gfs_fields:field_resol: untested code")
...
! North-south interpolation
  if (jy==1 .or. jy==fld%ny) then
    ...
    if (ib>rhs%ny) call abor1_ftn("fv3gfs_fields:field_resol: ib too large")
    do jx=1,rhs%nx
      do jf=1,rhs%nl*rhs%nf
...
if (iread==0) then
  call log%warning("fv3gfs_fields:read_file: Inventing State")
  ...
  WRITE(buf,*) 'fv3gfs_field:read_file: opening '//filename
  call log%info(buf)
  ...
  if (ix /= fld%nx .or. iy /= fld%ny .or. il /= fld%nl) then
    write (record,*) "fv3gfs_fields:read_file: ", &
                   & "input fields have wrong dimensions: ",ix,iy,il
    call log%error(record)
    write (record,*) "fv3gfs_fields:read_file: expected: ",fld%nx,fld%ny,fld%nl
    call log%error(record)
    call abor1_ftn("fv3gfs_fields:read_file: input fields have wrong dimensions")
  endif
...
WRITE(buf,*) 'fv3gfs_field:write_file: writing '//filename
call log%info(buf)
...
if (jj /= nf) call abor1_ftn("fv3gfs_fields_gpnorm: error number of fields")
...
if (lenfn>length) &
  & call abor1_ftn("fv3gfs_fields:genfilename: filename too long")
...
implicit none
type(fv3gfs_field), intent(in) :: x1, x2
integer :: common_vars
integer :: jf
...
if (x1%nx /= x2%nx .or.  x1%ny /= x2%ny .or.  x1%nl /= x2%nl) then
  call abor1_ftn ("fv3gfs_fields: resolution error")
endif
call check(x1)
call check(x2)
...
implicit none
type(fv3gfs_field), intent(in) :: self
logical :: bad
...
if (bad) then
  ...
  call abor1_ftn ("fv3gfs_fields: field not consistent")
endif
